...
data = driver.find_element(
    By.XPATH, "//table[@name='BookTable']//tr[5]/td[1]").text
print(data)
...
for row in range(2, rows_count+1):
    for column in range(1, columns_count+1):
        data = driver.find_element(
            By.XPATH, "//table[@name='BookTable']//tr[{}]/td[{}]".format(row, column)).text
        print(data, " | ", sep="", end="")
    print()
...
for row in range(2, rows_count+1):
    author = driver.find_element(
        By.XPATH, "//table[@name='BookTable']//tr[{}]/td[2]".format(row)).text
    if author == 'Mukesh':
        title = driver.find_element(
            By.XPATH, "//table[@name='BookTable']//tr[{}]/td[1]".format(row)).text
        print(title, author)
